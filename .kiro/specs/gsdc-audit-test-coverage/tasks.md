...
- [ ] 2. Production Fix — LibReentrancyGuard (Req 44)
...
- [ ] 3. Production Fix — Wire PausableFacet (Req 36)
  - [~] 3.1 Add `SystemPaused()` error and pause gate to `SettlementExecutorFacet`
  ...
  - [~] 3.2 Add `SystemPaused()` error and pause gate to `FloatManagerFacet`
...
- [ ] 4. Production Fix — Two-Step Admin Transfer (Req 37)
  - [~] 4.1 Add `transferAdmin` and `acceptAdmin` to `TimeLockControllerFacet`
...
- [ ] 5. Production Fix — maxQuoteTTL enforcement (Req 38)
  - [~] 5.1 Add `QuoteTTLExceeded` error and TTL check to `QuoteVerifierFacet`
...
- [ ] 6. Production Fix — Dead Storage Cleanup (Req 39)
  - [~] 6.1 Replace `usedNonces` with reserved sentinel in `LibSettlement.DiamondStorage`
...
- [ ] 7. Production Fix — corridorId in window error (Req 40)
  - [~] 7.1 Update `_enforceWindow` signature and both call sites in `SettlementExecutorFacet`
...
- [ ] 8. Production Fix — ERC-165 registration and DiamondInit guard (Reqs 41–42)
  - [~] 8.1 Register ERC-165 interface IDs in `DiamondInit.init`
  ...
  - [~] 8.2 Add `initialised` sentinel and re-init guard to `DiamondInit`
...
- [ ] 9. Production Fix — disputeSettlement access control and ISettlementDiamond completeness (Reqs 43, 45)
  - [~] 9.1 Restrict `DisputeResolverFacet.disputeSettlement` to LPs and orchestrator
  ...
  - [~] 9.2 Add missing functions to `ISettlementDiamond`
...
- [~] 10. Compile all contract changes and verify no regressions
...
- [ ] 11. Write `test/SecurityAudit.test.ts` — core audit tests (Reqs 1–14, 23–24)
  - [~] 11.1 System functionality & DiamondInit branch coverage
  ...
  - [~] 11.2 Access control: admin and orchestrator reverts (Req 2)
  ...
  - [~] 11.4 Write unit tests for security audit access-control edge cases
    - Test each `enforceAdmin`-gated function from a non-admin account
    - _Requirements: 2.6, 2.7_


- [ ] 12. Write `test/SecurityAudit.test.ts` — EIP-712 and EIP-3009 tests (Reqs 4–5)
  - [~] 12.1 EIP-712 signature integrity tests
  ...
  - [~] 12.2 EIP-3009 authorization integrity tests
...
- [ ] 13. Write time-lock and settlement guard tests (Reqs 6–9)
  - [~] 13.1 Write `test/TimeLock.test.ts` — time-lock integrity and new admin transfer
  ...
  - [~] 13.2 Write `test/SettlementWindow.test.ts` — settlement state machine and window tests
...
- [ ] 14. Write float, compliance, dispute, and misc coverage tests (Reqs 8–14, 17, 20–22, 25–27)
  - [~] 14.1 Write float reservation tests in `test/BranchCoverage.test.ts` (extend existing file)
  ...
  - [~] 14.2 Compliance, dispute, mint/burn, and margin tests
...
- [ ] 15. Write `test/PausableFacet.test.ts` (Reqs 11, 16, 36)
  - [~] 15.1 Implement full PausableFacet test file
...
- [ ] 16. Write `test/ProductionFixes.test.ts` (Reqs 36–45 one-to-one)
  - [~] 16.1 Req 36: `executeSettlementAggregated` and `reserveFloat` pause gate tests
  ...
  - [~] 16.2 Req 38: maxQuoteTTL enforcement tests
  ...
  - [~] 16.3 Req 39–42: Dead storage, corridorId error, ERC-165, re-init guard
  ...
  - [~] 16.4 Req 43–45: disputeSettlement AC, LibReentrancyGuard slot, interface completeness
...
- [ ] 17. Write `test/AdvancedVulnerability.test.ts` (Reqs 33–35)
  - [~] 17.1 Integer overflow and zero-BPS boundary tests
  ...
  - [~] 17.2 Reentrancy attack via malicious token and cross-contract independence
  ...
  - [~] 17.3 Front-running resistance tests
...
- [ ] 18. Write `test/PropertyBased.test.ts` — all nine PBT properties (Reqs 28–32)
  - [~] 18.1 Property 1: Margin BPS sum invariant (200 runs)
  ...
  - [~] 18.2 Write property test for Property 1 (margin BPS sum)
    - Implemented in 18.1 above; this sub-task marks the PBT status
    - **Property 1: Margin BPS Sum Invariant**
    - **Validates: Requirements 28.1, 28.2, 28.3, 28.4**
  - [~] 18.3 Property 2: Float reserve boundary rejection (200 runs)
  ...
  - [~] 18.4 Write property test for Property 2 (float boundary rejection)
    - **Property 2: Float Reserve Boundary Rejection**
    - **Validates: Requirements 29.2, 8.5**
  - [~] 18.5 Property 3: Float round-trip and release idempotence (200 runs)
  ...
  - [~] 18.6 Write property test for Property 3 (float round-trip)
    - **Property 3: Float Reservation Round-Trip**
    - **Validates: Requirements 29.1, 29.3, 29.4, 8.4, 8.6, 8.7**


  - [~] 18.7 Property 4: Settlement state machine forward-only (100 runs)
  ...
  - [~] 18.8 Write property test for Property 4 (settlement state machine)
    - **Property 4: Settlement State Machine Forward-Only Transition**
    - **Validates: Requirements 30.1, 30.2, 30.3, 30.4, 7.1, 7.12**
  - [~] 18.9 Property 5: EIP-712 signing round-trip (100 runs)
  ...
  - [~] 18.10 Write property test for Property 5 (EIP-712 round-trip)
    - **Property 5: EIP-712 Signing Round-Trip**
    - **Validates: Requirements 31.2, 31.3, 4.1, 4.11**
  - [~] 18.11 Property 6: EIP-3009 nonce anti-replay (100 runs)
  ...
  - [~] 18.12 Write property test for Property 6 (EIP-3009 anti-replay)
    - **Property 6: EIP-3009 Nonce Anti-Replay**
    - **Validates: Requirements 32.1, 32.2, 5.1, 5.7**


  - [~] 18.13 Property 7: Pause-unpause round-trip restores settlement (100 runs)
  ...
  - [~] 18.14 Write property test for Property 7 (pause round-trip)
    - **Property 7: Pause-Unpause Round-Trip Restores Settlement Capability**
    - **Validates: Requirements 36.4, 36.7**
  - [~] 18.15 Property 8: Quote TTL enforcement (200 runs)
  ...
  - [~] 18.16 Write property test for Property 8 (quote TTL)
    - **Property 8: Quote TTL Enforcement**
    - **Validates: Requirements 38.1, 38.2, 38.3**
  - [~] 18.17 Property 9: OutsideSettlementWindow carries actual corridorId (100 runs)
  ...
  - [~] 18.18 Write property test for Property 9 (corridorId in window error)
    - **Property 9: OutsideSettlementWindow Carries Actual CorridorId**
    - **Validates: Requirements 40.1, 40.2, 40.3, 7.5, 7.6, 7.10**

- [~] 19. Checkpoint — Ensure all unit and property-based tests pass
...
- [ ] 20. Update `.solcover.js` and coverage configuration (Req 15)
  - [~] 20.1 Set 100 % coverage gates in `.solcover.js` and `hardhat.config.ts`
...
- [~] 21. Final checkpoint — 100 % coverage gate and clean baseline
